...
class SessionsSheetFragment : LocaleAwareFragment(), View.OnClickListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val sessionView = view.findViewById<RecyclerView>(R.id.sessions)
        sessionView.layoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
        sessionView.adapter = sessionsAdapter
    ...
    private fun playAnimation(reverse: Boolean): Animator {
        ...
        val fullRadius = Math.sqrt(
                Math.pow(cardView!!.width.toDouble(), 2.0) + Math.pow(cardView!!.height.toDouble(), 2.0)).toFloat()
        ...
        sheetAnimator.duration = ANIMATION_DURATION.toLong()
        sheetAnimator.interpolator = AccelerateInterpolator()
        sheetAnimator.addListener(object : AnimatorListenerAdapter() {
    ...
    /* package */ internal fun animateAndDismiss(): Animator {

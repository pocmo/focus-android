...
object WebContextMenu {
    ...
    fun show(context: Context, callback: IWebView.Callback, hitTarget: IWebView.HitTarget) {
        ...
        if (hitTarget.isLink) {
            ...
            throw IllegalStateException("Unhandled long press target type")
        }
        ...
        builder.setOnCancelListener {
            ...
            val value: BrowserContextMenuValue
     ...
     * menu callbacks.
     */
    private fun setupMenuForHitTarget(dialog: Dialog,
        ...
        navigationView.menu.findItem(R.id.menu_new_tab).isVisible = hitTarget.isLink
        navigationView.menu.findItem(R.id.menu_link_share).isVisible = hitTarget.isLink
        navigationView.menu.findItem(R.id.menu_link_copy).isVisible = hitTarget.isLink
        navigationView.menu.findItem(R.id.menu_image_share).isVisible = hitTarget.isImage
        navigationView.menu.findItem(R.id.menu_image_copy).isVisible = hitTarget.isImage

        navigationView.menu.findItem(R.id.menu_image_save).isVisible = hitTarget.isImage && UrlUtils.isHttpOrHttps(hitTarget.imageURL)
        ...
        navigationView.setNavigationItemSelectedListener(NavigationView.OnNavigationItemSelectedListener { item ->
            ...
            when (item.itemId) {
                R.id.menu_new_tab -> {
                    val session = Session(hitTarget.linkURL, Session.Source.MENU)
                ...
                R.id.menu_link_share -> {
                    ...
                    dialog.context.startActivity(Intent.createChooser(shareIntent, dialog.context.getString(R.string.share_dialog_title)))
                    true
                }
                ...
                R.id.menu_image_save -> {
                    val download = Download(hitTarget.imageURL, null, null, null, -1, Environment.DIRECTORY_PICTURES, null)
                ...
                R.id.menu_link_copy, R.id.menu_image_copy -> {
                    ...
                    if (item.itemId == R.id.menu_link_copy) {
                        TelemetryWrapper.copyLinkEvent()
                        uri = Uri.parse(hitTarget.linkURL)
                    } else if (item.itemId == R.id.menu_image_copy) {
                        TelemetryWrapper.copyImageEvent()
                        uri = Uri.parse(hitTarget.imageURL)
                    } else {
                        throw IllegalStateException("Unknown hitTarget type - cannot copy to clipboard")
                    }
